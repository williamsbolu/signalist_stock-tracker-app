  ...
  async ({ event, step }) => {
    ...
    await step.run("send-welcome-email", async () => {
      ...
      const introText =
        ...
        "Thanks for joining Signalist. You now have the tools to track markets";

      const { email, name } = event.data;

      // Send the email
      return await sendWelcomeEmail({ email, intro: introText, name });
    });

    return { success: true, message: "Welcome email set successfully" };
  }
);
